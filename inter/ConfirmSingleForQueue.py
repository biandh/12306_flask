...
class confirmSingleForQueue:
    ...
    def sendConfirmSingleForQueue(self, log_info=[]):
        """
        # 模拟查询当前的列车排队人数的方法
        # 返回信息组成的提示字符串
        :return:
        ...
        try:
            ...
            if "status" in checkQueueOrderResult and checkQueueOrderResult["status"]:
                ...
                if 'submitStatus' in c_data and c_data['submitStatus'] is True:
                    log_info.append(u"提交订单成功！")
                    print(u"提交订单成功！")
                    qow = queryOrderWaitTime(self.session)
                    qow.sendQueryOrderWaitTime(log_info)
                else:
                    if 'errMsg' in c_data and c_data['errMsg']:
                        log_info.append(u"提交订单失败，{0}".format(c_data['errMsg']))
                        print(u"提交订单失败，{0}".format(c_data['errMsg']))
                    else:
                        log_info.append(c_data)
                        log_info.append(u'订票失败!很抱歉,请重试提交预订功能!')
                        print(c_data)
                        print(u'订票失败!很抱歉,请重试提交预订功能!')
            elif "messages" in checkQueueOrderResult and checkQueueOrderResult["messages"]:
                log_info.append(u"提交订单失败,错误信息: " + checkQueueOrderResult["messages"])
                print(u"提交订单失败,错误信息: " + checkQueueOrderResult["messages"])
            else:
                log_info.append(u"提交订单中，请耐心等待：" + checkQueueOrderResult["message"])
                print(u"提交订单中，请耐心等待：" + checkQueueOrderResult["message"])
        except ValueError:
            log_info.append(u"接口 {} 无响应".format(checkQueueOrderUrl))
